...
static void InitNames() {
	...
	g_r_names[R_CONTEXT_STATE]  = "R_CONTEXT_STATE";
		...
		g_names[IT_SET_SH_REG_INDIRECT]       = "IT_SET_SH_REG_INDIRECT";
		g_names[IT_SET_UCONFIG_REG_INDIRECT]  = "IT_SET_UCONFIG_REG_INDIRECT";
		...
		g_names[IT_SET_UCONFIG_REG_INDEX]     = "IT_SET_UCONFIG_REG_INDEX";
		...
		g_names[IT_SET_CONTEXT_REG_INDIRECT]  = "IT_SET_CONTEXT_REG_INDIRECT";
	...
	while (dw != 0) {
		...
		if ((cmd_id & 0xC0000000u) == 0xC0000000u) {
			...
			file->Printf("%s %s(OP:0x%02" PRIx8 ") SH:%s CNT:%u\n", g_names[op],
			             (op == IT_NOP ? g_r_names[r] : ""), op, sh_gx ? "GX" : "CX", len);
