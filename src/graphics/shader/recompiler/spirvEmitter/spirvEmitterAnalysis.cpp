...
DescriptorResourceBinding ResourceForDescriptor(const EmitterState&       state,
	...
	if (descriptor == nullptr) {
		...
		// bind kind that the layout phase didn't allocate (e.g. a CS
		// calling MIMG with an image the ShaderInfoCollection step thought
		// was unused). Demote the EXIT to a warning and return an empty
		// binding descriptor so the SPIR-V emitter skips the offending
		// descriptor set binding instead of killing the emulator.
	...
	if (found == descriptor->resources.end()) {
		...
		// but didn't include this particular resource id. Common when the
		// shader pre-allocates the descriptor set but never references it
		// in live instructions (the resource got pruned but the binding
		// didn't). Skip + warn.
