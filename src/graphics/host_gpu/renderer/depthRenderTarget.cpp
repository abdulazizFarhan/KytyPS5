...
static VkStencilOp ConvertStencilOp(uint8_t value, uint8_t write_mask, uint8_t op_value) {
	if (write_mask == 0) {
...
void ResolveRenderDepthTarget(uint64_t submit_id, CommandBuffer* buffer, const HW::Context& hw,
	...
	if (r->format == VK_FORMAT_UNDEFINED) {
		DepthFatal("no host depth/stencil format supports required usage for %s",
		           string_VkFormat(ideal_format).c_str());
	}
	...
	if (r->stencil_test_enable) {
		...
		r->stencil_static_front = {
		    ...
		    static_cast<VkCompareOp>(dc.stencilfunc)};
		...
		if (dc.backface_enable) {
			r->stencil_static_back = {
			    ...
			    static_cast<VkCompareOp>(dc.stencilfunc_bf)};
