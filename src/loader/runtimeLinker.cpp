	...
	// Result: GTA V reaches Window shown (vs FailFast exit 321 at 6.32s baseline)
	{
		...
		if (info->access_violation_type == Common::HostException::AccessViolationType::Read &&
			...
			if (!hd_watchdog_set.exchange(true)) {
				...
				// Cycle 0141if: Allow disabling RAGE termination via GTAV_NO_RAGE_TERMINATE env var
				int watchdog_ms = 500;
				...
				const char* no_term_env = std::getenv("GTAV_NO_RAGE_TERMINATE");
				const bool no_terminate = no_term_env != nullptr && no_term_env[0] != '0' && no_term_env[0] != 0;
			if (no_terminate) {
					LOGF("[0141if] GTAV_NO_RAGE_TERMINATE active - watchdog will NOT terminate RAGE\n");
				} else {
...
static void PatchProgram(Program* program, uint64_t address, uint64_t size) {
	...
	// // // // // // // Cycle 0141db: GTA V launcher code area dump (find actual code)
	// Dump bytes 0-0x200 to identify where the actual launcher code is
	// (vs data tables at file_off 0x70-0xb8).
	{
		const uint64_t launcher_start = 0x0ULL;
		const uint64_t launcher_size = 0x100ULL;
		if (launcher_start + launcher_size <= size) {
			auto* ptr = reinterpret_cast<uint8_t*>(address) + launcher_start;
			LOGF("Cycle 0141db: GTA V binary start area file_off 0x0-0x%" PRIx64 " (decrypted):\n", launcher_size);
